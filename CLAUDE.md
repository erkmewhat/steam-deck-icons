...
- Each action = one `.ts` file in `plugin/src/actions/`, compiled to `.js` in `.sdPlugin/bin/actions/`
- **Icons** are SVGs in icon pack `icons/` dirs, synced to plugin `imgs/actions/` by `tools/sync-icons.js`
- **Profiles** are generated by `tools/generate-profile[-game].py` with embedded base64 SVG icons
- **Hotkeys** sent via Windows SendInput (koffi FFI) with scan codes
- Manifests define UUIDs — action UUIDs in profile MUST exactly match plugin manifest
...
- **SD profile Image field is unreliable** — if icons don't show after one attempt, tell user to assign manually via Stream Deck UI (select button → + → icon library). Don't waste time re-embedding.
- **Plugin SVGs must be simple** — complex shapes (multiple circles, strokes) may not render. Keep to text + basic filled paths.
- **UUID mismatches silently fail** — always run `validate-profile.py` after profile generation.
