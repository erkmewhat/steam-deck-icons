...
function pressureColor(kpa: number, optMin: number, optMax: number): string {
    if (kpa < optMin - 5) return "#2288ff";
    if (kpa < optMin) return "#55bbff";
    if (kpa <= optMax) return "#00ee77";
    if (kpa <= optMax + 5) return "#ffcc00";
    return "#ff3333";
}

function tempColor(celsius: number): string {
    if (celsius < 70) return "#2288ff";
    if (celsius < 80) return "#55bbff";
    if (celsius <= 95) return "#00ee77";
    if (celsius <= 105) return "#ffcc00";
    return "#ff3333";
}
...
function batteryColor(fraction: number): string {
    if (fraction > 0.8) return "#2288ff";
    if (fraction > 0.4) return "#00ee77";
    if (fraction > 0.2) return "#ffcc00";
    return "#ff3333";
}
...
function renderTireGrid(title: string, cells: [TireCell, TireCell, TireCell, TireCell]): string {
...
export function renderTirePressure(pressures: [number, number, number, number], optMinKpa: number, optMaxKpa: number): string {
    ...
    })) as [TireCell, TireCell, TireCell, TireCell];
