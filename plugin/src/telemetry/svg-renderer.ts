...
function rpmColor(pct: number): string {
    if (pct < 0.5) return "#2266cc";       // low RPM — blue
    if (pct < 0.7) return "#44aaff";       // mid RPM — light blue
    if (pct < 0.85) return "#ffdd00";      // high RPM — yellow
    if (pct < 0.95) return "#ff8800";      // very high — orange
 ...
 */
export function renderGear(gear: number, rpmPct: number): string {
