...
cat <<HOOKEOF
{"systemMessage": "MANDATORY FIRST ACTION: Your very first response to the user MUST be the session greeting. Do NOT skip this. Do NOT respond to the user's message first. Show the greeting BEFORE anything else. Format: **Stream Deck Sim Racing — $BRANCH @ $LATEST_TAG** then Recent accomplishments (from git log), Up next on roadmap (from CLAUDE.md), and end with What do you want to work on? --- PROJECT STATE: branch=$BRANCH game=$CURRENT_GAME version=$LATEST_TAG commits_since_tag=$COMMITS_SINCE behind_remote=$BEHIND on_main=$ON_MAIN last_commit=$LAST_COMMIT_DATE", "hookSpecificOutput": {"hookEventName": "SessionStart", "additionalContext": "PROJECT STATE: branch=$BRANCH game=$CURRENT_GAME version=$LATEST_TAG commits_since_tag=$COMMITS_SINCE behind_remote=$BEHIND on_main=$ON_MAIN branches=$ALL_BRANCHES last_commit_date=$LAST_COMMIT_DATE quality_checklist=$CHECKLIST_EXISTS recent_commits=$RECENT_COMMITS --- MANDATORY SESSION START PROCEDURE: 1) If on main, switch to develop immediately. 2) If behind remote, pull. 3) Read CLAUDE.md for full roadmap and status. 4) Read tools/quality-checklist.md for self-check rules. 5) Present greeting to user: recent accomplishments (from git log and CLAUDE.md status), whats next on roadmap, then ask: What do you want to work on? 6) Run quality checklist silently before every major action."}}
HOOKEOF
